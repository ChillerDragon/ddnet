...
int CInput::Update()
{
	...
	int i = SDL_GetMouseState(NULL, NULL);
	if(i & SDL_BUTTON(1))
		m_aInputState[TW_KEY_MOUSE_1] = 1; // 1 is left
	if(i & SDL_BUTTON(3))
		m_aInputState[TW_KEY_MOUSE_2] = 1; // 3 is right
	if(i & SDL_BUTTON(2))
		m_aInputState[TW_KEY_MOUSE_3] = 1; // 2 is middle
	if(i & SDL_BUTTON(4))
		m_aInputState[TW_KEY_MOUSE_4] = 1;
	if(i & SDL_BUTTON(5))
		m_aInputState[TW_KEY_MOUSE_5] = 1;
	if(i & SDL_BUTTON(6))
		m_aInputState[TW_KEY_MOUSE_6] = 1;
	if(i & SDL_BUTTON(7))
		m_aInputState[TW_KEY_MOUSE_7] = 1;
	if(i & SDL_BUTTON(8))
		m_aInputState[TW_KEY_MOUSE_8] = 1;
	if(i & SDL_BUTTON(9))
		m_aInputState[TW_KEY_MOUSE_9] = 1;
		...
		while(SDL_PollEvent(&Event))
		{
			...
			case SDL_MOUSEBUTTONDOWN:
				if(Event.button.button == SDL_BUTTON_LEFT)
					Scancode = TW_KEY_MOUSE_1; // ignore_convention
				if(Event.button.button == SDL_BUTTON_RIGHT)
					Scancode = TW_KEY_MOUSE_2; // ignore_convention
				if(Event.button.button == SDL_BUTTON_MIDDLE)
					Scancode = TW_KEY_MOUSE_3; // ignore_convention
				if(Event.button.button == SDL_BUTTON_X1)
					Scancode = TW_KEY_MOUSE_4; // ignore_convention
				if(Event.button.button == SDL_BUTTON_X2)
					Scancode = TW_KEY_MOUSE_5; // ignore_convention
				if(Event.button.button == 6)
					Scancode = TW_KEY_MOUSE_6; // ignore_convention
				if(Event.button.button == 7)
					Scancode = TW_KEY_MOUSE_7; // ignore_convention
				if(Event.button.button == 8)
					Scancode = TW_KEY_MOUSE_8; // ignore_convention
				if(Event.button.button == 9)
					Scancode = TW_KEY_MOUSE_9; // ignore_convention
				break;

			case SDL_MOUSEWHEEL:
				if(Event.wheel.y > 0)
					Scancode = TW_KEY_MOUSE_WHEEL_UP; // ignore_convention
				if(Event.wheel.y < 0)
					Scancode = TW_KEY_MOUSE_WHEEL_DOWN; // ignore_convention
				if(Event.wheel.x > 0)
					Scancode = TW_KEY_MOUSE_WHEEL_LEFT; // ignore_convention
				if(Event.wheel.x < 0)
					Scancode = TW_KEY_MOUSE_WHEEL_RIGHT; // ignore_convention
				Action |= IInput::FLAG_RELEASE;
				break;
			...
			if(Scancode > TW_KEY_FIRST && Scancode < g_MaxKeys && !IgnoreKeys && (!SDL_IsTextInputActive() || m_EditingTextLen == -1))
			{
