			...
			if(GameTick >= m_aCurrentRecvTick[Conn] && GameTick > m_aAckGameTick[Conn])
			{
					...
					(NumParts == CSnapshot::MAX_PARTS && m_aSnapshotParts[Conn] == std::numeric_limits<uint64_t>::max()))
				{
					...
					if(!Dummy)
					{
						...
						if(IsSixup())
						{
							DemoSnapSize = GameClient()->OnDemoRecSnap7(pTmpBuffer3, pSnapSeven);
							if(DemoSnapSize < 0)
							{
...
void CClient::OnDemoPlayerSnapshot(void *pData, int Size)
{
	...
	if(IsSixup())
	{
		...
		TranslatedSize = GameClient()->TranslateSnap(pTmpTranslateBuffer, (CSnapshot *)pData, CONN_MAIN, false);
		if(TranslatedSize < 0)
		{
