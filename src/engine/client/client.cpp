...
void *CClient::SnapGetItem(int SnapID, int Index, CSnapItem *pItem) const
{
	...
	const CSnapshot *pSnapshot = m_aapSnapshots[g_Config.m_ClDummy][SnapID]->AltSnap();
...
void CClient::ProcessServerPacket(CNetChunk *pPacket, int Conn, bool Dummy)
{
	...
	if(IsSixup())
	{
		int Success = !TranslateSysMsg(&Msg, Sys, &Unpacker, Packer6, pPacket);
		if(Msg < 0)
			return;
		if(Success)
		{
			...
			if(GameTick >= m_aCurrentRecvTick[Conn] && GameTick > m_aAckGameTick[Conn])
			{
					...
					(NumParts == CSnapshot::MAX_PARTS && m_aSnapshotParts[Conn] == std::numeric_limits<uint64_t>::max()))
				{
					...
					if(IsSixup())
					{
						...
						unsigned char aAltTransSnapBuffer[CSnapshot::MAX_SIZE];
						CSnapshot *pAltTransSnapBuffer = (CSnapshot *)aAltTransSnapBuffer;
						// TODO: remove this expensive mem copy
						//       it was added so the translation layer can modify the snapshot
						//       without affecting future delta unpack errors
						//       the modification is a convienience for invalidating items
						//
						//       a nicer solution would be to either not modify the snap it is called on
						//       or use the pAltSnapBuffer we already have
						//       or use some kind of memory hack that is faster than mem copy
						mem_copy(pAltTransSnapBuffer, pTmpBuffer3, CSnapshot::MAX_SIZE);
						TranslatedSize = pAltTransSnapBuffer->TranslateSevenToSix(
							pTmpTranslateBuffer,
...
void CClient::OnDemoPlayerSnapshot(void *pData, int Size)
{
	...
	{
		pTmpTranslateBuffer = (CSnapshot *)aTmpTranslateBuffer;
		TranslatedSize = ((CSnapshot *)pData)->TranslateSevenToSix(pTmpTranslateBuffer, m_TranslationContext, LocalTime(), GameTick(g_Config.m_ClDummy), GameClient()->GetNetObjHandler7());
		if(TranslatedSize < 0)
		{
			dbg_msg("sixup", "failed to translate snapshot. error=%d", TranslatedSize);
			pTmpTranslateBuffer = nullptr;
	...
	if(pTmpTranslateBuffer && TranslatedSize > 0)
		mem_copy(m_aapSnapshots[g_Config.m_ClDummy][SNAP_CURRENT]->m_pTransSnap, pTmpTranslateBuffer, TranslatedSize);
...
const char *CClient::DemoPlayer_Play(const char *pFilename, int StorageType)
{
	...
	for(int SnapshotType = 0; SnapshotType < NUM_SNAPSHOT_TYPES; SnapshotType++)
	{
		...
		m_aapSnapshots[g_Config.m_ClDummy][SnapshotType]->m_pTransSnap = (CSnapshot *)&m_aaaDemorecSnapshotData[SnapshotType][2];
		...
		m_aapSnapshots[g_Config.m_ClDummy][SnapshotType]->m_TransSnapSize = 0;
