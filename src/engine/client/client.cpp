...
void CClient::ProcessServerPacket(CNetChunk *pPacket, int Conn, bool Dummy)
{
	...
	if(Sys)
	{
		...
		else if(IsSixup() && Conn == CONN_DUMMY && (pPacket->m_Flags & NET_CHUNKFLAG_VITAL) != 0 && Msg == NETMSG_MAP_CHANGE)
		{
			m_DummySendConnInfo = true;
		}
...
void CClient::OnDemoPlayerSnapshot(void *pData, int Size)
{
	...
	if(IsSixup())
	{
		...
		TranslatedSize = ((CSnapshot *)pData)->TranslateSevenToSix(pTmpTranslateBuffer, m_TranslationContext, LocalTime(), GameTick(g_Config.m_ClDummy), CONN_MAIN, false, GameClient()->GetNetObjHandler7());
		if(TranslatedSize < 0)
		{
	...
	if(pTmpTranslateBuffer && TranslatedSize > 0)
		mem_copy(m_aapSnapshots[g_Config.m_ClDummy][SNAP_CURRENT]->m_pAltSnap, pTmpTranslateBuffer, TranslatedSize);
