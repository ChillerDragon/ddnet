...
void CClient::Run()
{
	...
	*/
	...
	while(1)
	{
		...
		if(CtrlShiftKey(TW_KEY_D, LastD))
			g_Config.m_Debug ^= 1;

		if(CtrlShiftKey(TW_KEY_G, LastG))
			g_Config.m_DbgGraphs ^= 1;

		if(CtrlShiftKey(TW_KEY_E, LastE))
		{
...
{
	if(Input()->KeyIsPressed(TW_KEY_LCTRL) && Input()->KeyIsPressed(TW_KEY_LSHIFT) && !Last && Input()->KeyIsPressed(Key))
	{
		Last = true;
		return true;
	}
