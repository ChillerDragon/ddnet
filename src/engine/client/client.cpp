			...
			if(GameTick >= m_aCurrentRecvTick[Conn] && GameTick > m_aAckGameTick[Conn])
			{
					...
					(NumParts == CSnapshot::MAX_PARTS && m_aSnapshotParts[Conn] == std::numeric_limits<uint64_t>::max()))
				{
					...
					if(!Dummy)
					{
						...
						mem_copy(aExtraInfoRemoved, IsSixup() ? pAltSnapBuffer : pTmpBuffer3, IsSixup() ? AltSnapSize : SnapSize);
						...
						for(auto &DemoRecorder : m_aDemoRecorder)
						{
							...
							{
								// write snapshot
								DemoRecorder.RecordSnapshot(GameTick, aExtraInfoRemoved, IsSixup() ? AltSnapSize : SnapSize);
							}
						}
					}
