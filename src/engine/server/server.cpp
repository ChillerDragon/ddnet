...
void CServer::ConAccessLevel(IConsole::IResult *pResult, void *pUser)
{
	...
	CRconRole *pModerator = pManager->FindRole(ROLE_NAME_MODERATOR);
	CRconRole *pHelper = pManager->FindRole(ROLE_NAME_HELPER);
	...
	if(pResult->NumArguments() > 1)
	{
		...
		if(!pRole)
		{
			if(str_startswith(pRoleName, "mod"))
				pRole = pManager->FindRole(ROLE_NAME_MODERATOR);
		}
		if(!pRole)
		{
		...
		if(!str_comp(pRole->Name(), ROLE_NAME_ADMIN))
		{
			pModerator->DisallowCommand(pCommand);
			pHelper->DisallowCommand(pCommand);
		}
		else if(!str_comp(pRole->Name(), ROLE_NAME_MODERATOR))
		{
			pModerator->AllowCommand(pCommand);
			pHelper->DisallowCommand(pCommand);
		}
		else if(!str_comp(pRole->Name(), ROLE_NAME_HELPER))
		{
