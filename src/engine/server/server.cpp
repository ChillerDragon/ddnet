...
void CServer::ProcessClientPacket(CNetChunk *pPacket)
{
	...
	if(Sys)
	{
		...
		{
			const char *pOrigin = Unpacker.GetString();
			CUuid *pSessionId = (CUuid *)Unpacker.GetRaw(sizeof(*pSessionId));
			if(Unpacker.Error())
				return;

			OnNetMsgRedirectOrigin(ClientId, pOrigin, pSessionId);
		}
...
void CServer::OnNetMsgRedirectOrigin(int ClientId, const char *pOrigin, CUuid *pSessionId)
{
	log_info("server", "cid=%d got redirected from '%s'", ClientId, pOrigin);
