...
void CServer::ConRoleRank(IConsole::IResult *pResult, void *pUser)
{
	CServer *pThis = (CServer *)pUser;
	CAuthManager *pManager = &pThis->m_AuthManager;

	const char *pRoleName = pResult->GetString(0);
	int Rank = pResult->GetInteger(1);
	...
	if(pRole->SetRank(Rank))
	{
		log_info("auth", "Set role '%s' to rank %d.", pRoleName, Rank);
	}
	else
	{
		log_error("auth", "Failed to set rank for role '%s'.", pRoleName);
	}
}
...
void CServer::ConRoleCreate(IConsole::IResult *pResult, void *pUser)
{
	...
	const char *pRoleName = pResult->GetString(0);
	if(pManager->AddRole(pRoleName, RoleRank::HELPER))
	{
...
void CServer::RegisterCommands()
{
	...
	Console()->Register("role_rank", "s[role]i[rank]", CFGFLAG_SERVER, ConRoleRank, this, "Set the rank of a rcon role. Higher is stronger. Determines who can kick who.");
