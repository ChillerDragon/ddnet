...
public:
	...
	void Reset()
	{
		for(int &LocalClientID : m_LocalClientID)
			LocalClientID = -1;
	...
	const protocol7::CNetObj_PlayerInfoRace *m_paPlayerInfosRace[MAX_CLIENTS];
	CClientData m_aClients[MAX_CLIENTS];
	int m_aDamageTaken[MAX_CLIENTS];
	float m_aDamageTakenTick[MAX_CLIENTS];

	int m_LocalClientID[2]; // TODO: use NUM_DUMMIES (currently include breaks compile)
