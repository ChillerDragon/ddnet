	...
	protocol7::CNetObjHandler *pNetObjHandler)
{
	...
	for(auto &PlayerInfosRace : TranslationContext.m_paPlayerInfosRace)
	...
	for(int i = 0; i < m_NumItems; i++)
	{
		...
		if(pItem7->Type() == protocol7::NETOBJTYPE_PLAYERINFORACE)
		{
			...
			{
				TranslationContext.m_paPlayerInfosRace[ClientID] = pInfo;
			}
		}
	...
	if(TranslationContext.m_ShouldSendGameInfo)
	{
		...
		*/
		...
		int TimerClientID = clamp(TranslationContext.m_LocalClientID[Conn], 0, (int)MAX_CLIENTS);
		if(SpectatorID >= 0)
			TimerClientID = SpectatorID;
		const protocol7::CNetObj_PlayerInfoRace *pRaceInfo = TranslationContext.m_paPlayerInfosRace[TimerClientID];
		if(pRaceInfo)
		{
	...
	for(int i = 0; i < m_NumItems; i++)
	{
		...
		else if(pItem7->Type() == protocol7::NETOBJTYPE_PLAYERINFO)
		{
			...
			Info6.m_Local = TranslationContext.m_LocalClientID[Conn] == pItem7->ID();
