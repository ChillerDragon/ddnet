...
void CNetBase::SendControlMsgWithToken7(NETSOCKET Socket, NETADDR *pAddr, TOKEN Token, int Ack, int ControlMsg, TOKEN MyToken, bool Extended)
{
	...
	static unsigned char s_aRequestTokenBuf[NET_TOKENREQUEST_DATASIZE];
	s_aRequestTokenBuf[3] = (MyToken >> 24) & 0xff;
	s_aRequestTokenBuf[2] = (MyToken >> 16) & 0xff;
	s_aRequestTokenBuf[1] = (MyToken >> 8) & 0xff;
	s_aRequestTokenBuf[0] = (MyToken)&0xff;
