...
{
	// dbg_msg("network_out", "CNetBase::SendPacket token=%x none=%x unsupported=%x sixup=%d", SecurityToken, NET_TOKEN_NONE, NET_SECURITY_TOKEN_UNSUPPORTED, Sixup);
	unsigned char aBuffer[NET_MAX_PACKETSIZE];
...
int CNetBase::UnpackPacket(unsigned char *pBuffer, int Size, CNetPacketConstruct *pPacket, bool &Sixup, SECURITY_TOKEN *pSecurityToken, SECURITY_TOKEN *pResponseToken)
{
	...
	if(pPacket->m_Flags & NET_PACKETFLAG_CONNLESS)
	{
		...
		if(Sixup)
		{
			...
			dbg_msg("network_in", "got token=%x", *pSecurityToken);
		}
	...
	if(pPacket->m_Flags & NET_PACKETFLAG_CONTROL)
	{
		if(pPacket->m_DataSize >= 5) // control byte + token
		{
			...
			{
				*pResponseToken = (pPacket->m_aChunkData[1] << 24) | (pPacket->m_aChunkData[2] << 16) | (pPacket->m_aChunkData[3] << 8) | pPacket->m_aChunkData[4];
			}
		}
	}
...
void CNetBase::SendControlMsgWithToken(NETSOCKET Socket, NETADDR *pAddr, TOKEN Token, int Ack, int ControlMsg, TOKEN MyToken, bool Extended)
{
	...
	SendControlMsg(Socket, pAddr, 0, ControlMsg, m_aRequestTokenBuf, Extended ? sizeof(m_aRequestTokenBuf) : 4, Token, true);
}
