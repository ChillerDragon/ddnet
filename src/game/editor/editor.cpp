...
int CEditor::UiDoValueSelector(void *pID, CUIRect *pRect, const char *pLabel, int Current, int Min, int Max, int Step, float Scale, const char *pToolTip, bool IsDegree, bool IsHex, int Corners, ColorRGBA *Color)
{
	...
	if(s_TextMode && s_LastTextpID == pID)
	{
		...
		if(Input()->KeyIsPressed(TW_KEY_RETURN) || Input()->KeyIsPressed(TW_KEY_KP_ENTER) ||
		...
		if(Input()->KeyIsPressed(TW_KEY_ESCAPE))
		{
			m_LockMouse = false;
			UI()->SetActiveItem(0);
			s_TextMode = false;
	...
	else
	{
		if(UI()->ActiveItem() == pID)
		{
			if(UI()->MouseButton(0))
			{
				if(Input()->KeyIsPressed(TW_KEY_LSHIFT) || Input()->KeyIsPressed(TW_KEY_RSHIFT))
...
{
	bool CtrlPressed = Input()->KeyIsPressed(TW_KEY_LCTRL) || Input()->KeyIsPressed(TW_KEY_RCTRL);
	bool ShiftPressed = Input()->KeyIsPressed(TW_KEY_LSHIFT) || Input()->KeyIsPressed(TW_KEY_RSHIFT);
	...
	// top line buttons
	{
		...
		if(DoButton_Editor(&s_HqButton, "HD", m_ShowDetail, &Button, 0, "[ctrl+h] Toggle High Detail") ||
			(Input()->KeyPress(TW_KEY_H) && CtrlPressed))
		{
		...
		if(DoButton_Editor(&s_AnimateButton, "Anim", m_Animate, &Button, 0, "[ctrl+m] Toggle animation") ||
			(Input()->KeyPress(TW_KEY_M) && CtrlPressed))
		{
		...
		if(DoButton_Editor(&s_ProofButton, "Proof", m_ProofBorders, &Button, 0, "[ctrl+p] Toggles proof borders. These borders represent what a player maximum can see.") ||
			(Input()->KeyPress(TW_KEY_P) && CtrlPressed))
		{
		...
		if(DoButton_Editor(&s_GridButton, "Grid", m_GridActive, &Button, 0, "[ctrl+g] Toggle Grid") ||
			(Input()->KeyPress(TW_KEY_G) && CtrlPressed))
		{
		...
		if(DoButton_Editor(&s_TileInfoButton, "Info", m_ShowTileInfo, &Button, 0, "[ctrl+i] Show tile information") ||
			(Input()->KeyPress(TW_KEY_I) && CtrlPressed))
		{
		...
		if(DoButton_Editor(&s_AllowPlaceUnusedTilesButton, "Unused", m_AllowPlaceUnusedTiles, &Button, 0, "[ctrl+u] Allow placing unused tiles") ||
			(Input()->KeyPress(TW_KEY_U) && CtrlPressed))
		{
		...
		// brush manipulation
		{
			...
			if(DoButton_Ex(&s_FlipXButton, "X/X", Enabled, &Button, 0, "[N] Flip brush horizontal", CUI::CORNER_L) || (Input()->KeyPress(TW_KEY_N) && m_Dialog == DIALOG_NONE && m_EditBoxActive == 0))
			{
			...
			if(DoButton_Ex(&s_FlipyButton, "Y/Y", Enabled, &Button, 0, "[M] Flip brush vertical", CUI::CORNER_R) || (Input()->KeyPress(TW_KEY_M) && m_Dialog == DIALOG_NONE && m_EditBoxActive == 0))
			{
				for(int i = 0; i < m_Brush.m_lLayers.size(); i++)
			...
			if(DoButton_Ex(&s_CcwButton, "CCW", Enabled, &Button, 0, "[R] Rotates the brush counter clockwise", CUI::CORNER_L) || (Input()->KeyPress(TW_KEY_R) && m_Dialog == DIALOG_NONE && m_EditBoxActive == 0))
			{
				for(int i = 0; i < m_Brush.m_lLayers.size(); i++)
			...
			if(DoButton_Ex(&s_CwButton, "CW", Enabled, &Button, 0, "[T] Rotates the brush clockwise", CUI::CORNER_R) || (Input()->KeyPress(TW_KEY_T) && m_Dialog == DIALOG_NONE && m_EditBoxActive == 0))
			{
				for(int i = 0; i < m_Brush.m_lLayers.size(); i++)
					m_Brush.m_lLayers[i]->BrushRotate(s_RotationAmount / 360.0f * pi * 2);
			}
	...
	// Bottom line buttons
	{
		// refocus button
		{
			...
			if(DoButton_Editor(&s_RefocusButton, "Refocus", m_WorldOffsetX && m_WorldOffsetY ? 0 : -1, &Button, 0, "[HOME] Restore map focus") || (m_EditBoxActive == 0 && Input()->KeyPress(TW_KEY_HOME)))
			{
				m_WorldOffsetX = 0;
				m_WorldOffsetY = 0;
			}
			...
			// do tele/tune/switch/speedup button
			{
				...
				if(pS)
				{
					...
					if(aButtonName != 0)
					{
						...
						if(DoButton_Ex(&s_ModifierButton, aButtonName, 0, &Button, 0, aBuf, CUI::CORNER_ALL) || (CtrlPressed && Input()->KeyPress(TW_KEY_A)))
						{
		...
		if(pLayer && (pLayer->m_Type == LAYERTYPE_QUADS || pLayer->m_Type == LAYERTYPE_SOUNDS))
		{
			...
			if(pLayer->m_Type == LAYERTYPE_QUADS)
			{
				Invoked = DoButton_Editor(&s_AddItemButton, "Add Quad", 0, &Button, 0, "[ctrl+q] Add a new quad") ||
					  (Input()->KeyPress(TW_KEY_Q) && CtrlPressed);
			}
			...
			if(Invoked)
			{
				...
				if(Input()->KeyPress(TW_KEY_Q) && CtrlPressed)
				{
		...
		// Brush draw mode button
		{
			...
			if(DoButton_Editor(&s_BrushDrawModeButton, "Destructive", m_BrushDrawDestructive, &Button, 0, "[ctrl+d] Toggle brush draw mode") ||
				(Input()->KeyPress(TW_KEY_D) && CtrlPressed && !ShiftPressed))
...
void CEditor::DoSoundSource(CSoundSource *pSource, int Index)
{
	...
	bool IgnoreGrid;
	if(Input()->KeyIsPressed(TW_KEY_LALT) || Input()->KeyIsPressed(TW_KEY_RALT))
		IgnoreGrid = true;
	else
		IgnoreGrid = false;
...
void CEditor::DoQuad(CQuad *q, int Index)
{
	...
	else if(UI()->HotItem() == pID)
	{
		...
		if(UI()->MouseButton(0))
		{
			...
			else if(Input()->KeyIsPressed(TW_KEY_LCTRL) || Input()->KeyIsPressed(TW_KEY_RCTRL))
			{
				m_LockMouse = true;
...
void CEditor::DoQuadEnvPoint(const CQuad *pQuad, int QIndex, int PIndex)
{
	...
	else if(UI()->HotItem() == pID && s_ActQIndex == QIndex)
	{
		...
		if(UI()->MouseButton(0))
		{
			if(Input()->KeyIsPressed(TW_KEY_LCTRL) || Input()->KeyIsPressed(TW_KEY_RCTRL))
			{
				m_LockMouse = true;
				s_Operation = OP_ROTATE;
...
void CEditor::DoMapEditor(CUIRect View)
{
	...
	if(Inside)
	{
		...
		if(UI()->ActiveItem() == 0 && (UI()->MouseButton(0) || UI()->MouseButton(2)))
		{
			...
			if(Input()->KeyIsPressed(TW_KEY_LCTRL) || Input()->KeyIsPressed(TW_KEY_RCTRL) || UI()->MouseButton(2))
			{
				if(Input()->KeyIsPressed(TW_KEY_LSHIFT))
		...
		if(UI()->HotItem() == s_pEditorID)
		{
			...
			else
			{
				...
				if(UI()->MouseButton(0) && s_Operation == OP_NONE)
				{
					...
					if((Input()->KeyIsPressed(TW_KEY_LSHIFT) || Input()->KeyIsPressed(TW_KEY_RSHIFT)) && pLayer)
...
int CEditor::DoProperties(CUIRect *pToolBox, CProperty *pProps, int *pIDs, int *pNewVal, ColorRGBA Color)
{
	...
	for(int i = 0; pProps[i].m_pName; i++)
	{
		...
		else if(pProps[i].m_Type == PROPTYPE_ANGLE_SCROLL)
		{
			...
			Shifter.VSplitRight(10.0f, &Shifter, &Inc);
			Shifter.VSplitLeft(10.0f, &Dec, &Shifter);
			bool Shift = Input()->KeyIsPressed(TW_KEY_LSHIFT) || Input()->KeyIsPressed(TW_KEY_RSHIFT);
			int Step = Shift ? 1 : 45;
...
void CEditor::RenderLayers(CUIRect ToolBox, CUIRect View)
{
	...
	if(LayersHeight > LayersBox.h) // Do we even need a scrollbar?
	{
		...
		if(UI()->MouseInside(&Scroll) || UI()->MouseInside(&LayersBox))
		{
			...
			if(ScrollNum > 0)
			{
				if(Input()->KeyPress(TW_KEY_MOUSE_WHEEL_UP))
					s_ScrollValue = clamp(s_ScrollValue - 1.0f / ScrollNum, 0.0f, 1.0f);
				if(Input()->KeyPress(TW_KEY_MOUSE_WHEEL_DOWN))
					s_ScrollValue = clamp(s_ScrollValue + 1.0f / ScrollNum, 0.0f, 1.0f);
			}
		}
	}
		...
		for(int g = 0; g < m_Map.m_lGroups.size(); g++)
		{
			...
			if(LayerCur >= LayerStartAt)
			{
					   ...
					   BUTTON_CONTEXT, m_Map.m_lGroups[g]->m_Collapse ? "Select group. Shift click to select all layers. Double click to expand." : "Select group. Shift click to select all layers. Double click to collapse.", CUI::CORNER_R, FontSize))
				{
					...
					if((Input()->KeyIsPressed(TW_KEY_LSHIFT) || Input()->KeyIsPressed(TW_KEY_RSHIFT)) && m_SelectedGroup == g)
					{
						m_lSelectedLayers.clear();
			...
			for(int i = 0; i < m_Map.m_lGroups[g]->m_lLayers.size(); i++)
			{
					   ...
					   BUTTON_CONTEXT, "Select layer. Shift click to select multiple.", CUI::CORNER_R, FontSize))
				{
					...
					if(Result == 1)
					{
						...
						else if(!(Input()->KeyIsPressed(TW_KEY_LSHIFT) || Input()->KeyIsPressed(TW_KEY_RSHIFT)))
						{
							SelectLayer(i, g);
						}
					}
	...
	if(Input()->KeyPress(TW_KEY_DOWN) && m_Dialog == DIALOG_NONE && m_EditBoxActive == 0)
	{
		if(Input()->KeyIsPressed(TW_KEY_LSHIFT) || Input()->KeyIsPressed(TW_KEY_RSHIFT))
		{
	...
	if(Input()->KeyPress(TW_KEY_UP) && m_Dialog == DIALOG_NONE && m_EditBoxActive == 0)
	{
		if(Input()->KeyIsPressed(TW_KEY_LSHIFT) || Input()->KeyIsPressed(TW_KEY_RSHIFT))
		{
...
void CEditor::RenderImages(CUIRect ToolBox, CUIRect View)
{
	...
	if(Input()->KeyPress(TW_KEY_DOWN) && m_Dialog == DIALOG_NONE)
	{
	...
	if(Input()->KeyPress(TW_KEY_UP) && m_Dialog == DIALOG_NONE)
	{
		int OldImage = m_SelectedImage;
		m_SelectedImage = clamp(m_SelectedImage, 0, m_Map.m_lImages.size() - 1);
...
void CEditor::RenderFileDialog()
{
	...
	if(m_FilesSelectedIndex > -1)
	{
		...
		for(int i = 0; i < Input()->NumEvents(); i++)
		{
			...
			{
				if(Input()->GetEvent(i).m_Key == TW_KEY_DOWN)
				{
				...
				if(Input()->GetEvent(i).m_Key == TW_KEY_UP)
				{
	...
	for(int i = 0; i < Input()->NumEvents(); i++)
	{
		if(Input()->GetEvent(i).m_Flags & IInput::FLAG_PRESS)
		{
			if(Input()->GetEvent(i).m_Key == TW_KEY_RETURN || Input()->GetEvent(i).m_Key == TW_KEY_KP_ENTER)
				m_FileDialogActivate = true;
		}
	}
	...
	ButtonBar.VSplitRight(50.0f, &ButtonBar, &Button);
	if(DoButton_Editor(&s_CancelButton, "Cancel", 0, &Button, 0, 0) || Input()->KeyIsPressed(TW_KEY_ESCAPE))
...
void CEditor::RenderModebar(CUIRect View)
{
	...
	// mode buttons
	{
		...
		if(MouseButton == 2 || (Input()->KeyPress(TW_KEY_LEFT) && m_Dialog == DIALOG_NONE && m_EditBoxActive == 0))
		{
			if(m_Mode == MODE_LAYERS)
				m_Mode = MODE_SOUNDS;
			else if(m_Mode == MODE_IMAGES)
		...
		else if(MouseButton == 1 || (Input()->KeyPress(TW_KEY_RIGHT) && m_Dialog == DIALOG_NONE && m_EditBoxActive == 0))
		{
			if(m_Mode == MODE_LAYERS)
				m_Mode = MODE_IMAGES;
			else if(m_Mode == MODE_IMAGES)
...

		{
			...
			for(int c = 0; c < pEnvelope->m_Channels; c++)
			{
				...
				for(int i = 0; i < pEnvelope->m_lPoints.size(); i++)
				{
					...
					if(pID == s_pID && (Input()->KeyIsPressed(TW_KEY_RETURN) || Input()->KeyIsPressed(TW_KEY_KP_ENTER)))
					{
						if(i != 0)
						{
...
void CEditor::RenderServerSettingsEditor(CUIRect View, bool ShowServerSettingsEditorLast)
{
	...
	// do the toolbar
	{
		...
		if(DoButton_Editor(&s_AddButton, "Add", 0, &Button, 0, "Add a command to command list.") || ((Input()->KeyPress(TW_KEY_RETURN) || Input()->KeyPress(TW_KEY_KP_ENTER)) && UI()->LastActiveItem() == &m_CommandBox))
		{
		...
		if(m_Map.m_lSettings.size() && s_CommandSelectedIndex > -1 && s_CommandSelectedIndex < m_Map.m_lSettings.size())
		{
			...
			if(DoButton_Editor(&s_ModButton, "Mod", 0, &Button, 0, "Modify a command from the command list.") || (Input()->KeyPress(TW_KEY_M) && UI()->LastActiveItem() != &m_CommandBox))
			{
			...
			Button.VSplitRight(5.0f, &Button, 0);
			static int s_DelButton = 0;
			if(DoButton_Editor(&s_DelButton, "Del", 0, &Button, 0, "Delete a command from the command list.") || (Input()->KeyPress(TW_KEY_DELETE) && UI()->LastActiveItem() != &m_CommandBox))
			{
				m_Map.m_lSettings.remove_index(s_CommandSelectedIndex);
...
void CEditor::RenderMenubar(CUIRect MenuBar)
{
	...
	if(DoButton_Editor(&s_CloseButton, "×", 0, &Close, 0, "Exits from the editor", 0) || (m_Dialog == DIALOG_NONE && !UiPopupOpen() && !m_PopupEventActivated && Input()->KeyPress(TW_KEY_ESCAPE)))
		g_Config.m_ClEditor = 0;
}
...
void CEditor::Render()
{
	...
	m_ShowPicker = Input()->KeyIsPressed(TW_KEY_SPACE) != 0 && m_Dialog == DIALOG_NONE && m_EditBoxActive == 0 && UI()->LastActiveItem() != &m_CommandBox && m_lSelectedLayers.size() == 1;
	...
	// do zooming
	if(Input()->KeyPress(TW_KEY_KP_MINUS) && m_Dialog == DIALOG_NONE && m_EditBoxActive == 0)
		m_ZoomLevel += 50;
	if(Input()->KeyPress(TW_KEY_KP_PLUS) && m_Dialog == DIALOG_NONE && m_EditBoxActive == 0)
		m_ZoomLevel -= 50;
	if(Input()->KeyPress(TW_KEY_KP_MULTIPLY) && m_Dialog == DIALOG_NONE && m_EditBoxActive == 0)
	{
		m_EditorOffsetX = 0;
		m_EditorOffsetY = 0;
		m_ZoomLevel = 100;
	...
	for(int i = TW_KEY_1; i <= TW_KEY_0; i++)
	{
		...
		{
			int Slot = i - TW_KEY_1;
			if((Input()->KeyIsPressed(TW_KEY_LCTRL) || Input()->KeyIsPressed(TW_KEY_RCTRL)) && !m_Brush.IsEmpty())
			{
	...
	if(m_Dialog == DIALOG_NONE)
	{
		bool CtrlPressed = Input()->KeyIsPressed(TW_KEY_LCTRL) || Input()->KeyIsPressed(TW_KEY_RCTRL);
		bool ShiftPressed = Input()->KeyIsPressed(TW_KEY_LSHIFT) || Input()->KeyIsPressed(TW_KEY_RSHIFT);
		bool AltPressed = Input()->KeyIsPressed(TW_KEY_LALT) || Input()->KeyIsPressed(TW_KEY_RALT);
		// ctrl+n to create new map
		if(Input()->KeyPress(TW_KEY_N) && CtrlPressed)
		{
			if(HasUnsavedData())
			{
...
void CEditor::UpdateAndRender()
{
	...
	float rx = 0, ry = 0;
	{
		...
		int Buttons = 0;
		if(Input()->KeyIsPressed(TW_KEY_MOUSE_1))
			Buttons |= 1;
		if(Input()->KeyIsPressed(TW_KEY_MOUSE_2))
			Buttons |= 2;
		if(Input()->KeyIsPressed(TW_KEY_MOUSE_3))
			Buttons |= 4;
	...
	// toggle gui
	if(Input()->KeyPress(TW_KEY_TAB))
		m_GuiActive = !m_GuiActive;

	if(Input()->KeyPress(TW_KEY_F10))
