	...
	for(int y = 0; y < pRedirectLayer->m_Height; y++)
		for(int x = 0; x < pRedirectLayer->m_Width; x++)
		{
			...
			if((m_pEditor->m_AllowPlaceUnusedTiles || IsValidRedirectTile(pRedirectLayer->m_pTiles[(y * pRedirectLayer->m_Width) + x].m_Index)) && pRedirectLayer->m_pTiles[y * pRedirectLayer->m_Width + x].m_Index != TILE_AIR)
			{
				...
				m_pRedirectTile[Index].m_Port = m_pEditor->m_RedirectPort;
				m_pRedirectTile[Index].m_Type = pRedirectLayer->m_pTiles[(y * pRedirectLayer->m_Width) + x].m_Index;
				m_pRedirectTile[Index].m_Reserved = 0;
				mem_zero(m_pRedirectTile[Index].m_aReservedForAddress, sizeof(m_pRedirectTile[Index].m_aReservedForAddress));
				m_pTiles[Index].m_Index = pRedirectLayer->m_pTiles[(y * pRedirectLayer->m_Width) + x].m_Index;
			}
			else
			{
				m_pRedirectTile[Index].m_Port = 0;
				m_pRedirectTile[Index].m_Type = 0;
				m_pRedirectTile[Index].m_Reserved = 0;
				mem_zero(m_pRedirectTile[Index].m_aReservedForAddress, sizeof(m_pRedirectTile[Index].m_aReservedForAddress));
	...
	for(int y = 0; y < h; y++)
	{
		for(int x = 0; x < w; x++)
		{
			...
			if(Empty || (!m_pEditor->m_AllowPlaceUnusedTiles && !IsValidRedirectTile((pLt->m_pTiles[SrcIndex]).m_Index))) // no redirect tile chosen: reset
			{
				...
				m_pRedirectTile[TgtIndex].m_Port = 0;
				m_pRedirectTile[TgtIndex].m_Type = 0;
			...
			else
			{
				...
				{
					m_pRedirectTile[TgtIndex].m_Type = m_pTiles[TgtIndex].m_Index;
					m_pRedirectTile[TgtIndex].m_Port = m_pEditor->m_RedirectPort;
				}
			}
