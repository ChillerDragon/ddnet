...
void CEditorBrushDrawAction::Apply(bool Undo)
{
	...
	for(auto const &RedirectChange : m_RedirectTileChanges)
	{
		...
		for(auto &Tile : Line)
		{
			...
			Map.m_pRedirectLayer->m_pRedirectTile[Index].m_Port = Data.m_Port;
			Map.m_pRedirectLayer->m_pRedirectTile[Index].m_Type = Data.m_Type;
			Map.m_pRedirectLayer->m_pTiles[Index].m_Index = Data.m_Index;

			dbg_msg("actions", "playing shit fucking type %d", Data.m_Type);
		}
	}
}
