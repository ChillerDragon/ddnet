...
void CRenderTools::RenderTee7(CAnimState *pAnim, const CTeeRenderInfo *pInfo, int Emote, vec2 Dir, vec2 Pos)
{
	vec2 Direction = Dir;
	vec2 Position = Pos;
	bool IsBot = pInfo->m_BotTexture >= 0;
		...
		for(int f = 0; f < 2; f++)
		{
			...
			if(f == 1)
			{
				...
				if(IsBot && !OutLine)
				{
					...
					SelectSprite(client_data7::SPRITE_TEE_BOT_BACKGROUND, 0, 0, 0);
					Item = BotItem;
				...
				if(IsBot && !OutLine)
				{
					...
					Graphics()->QuadsBegin();
					Graphics()->SetColor(1.0f, 1.0f, 1.0f, 1.0f);
					SelectSprite(client_data7::SPRITE_TEE_BOT_FOREGROUND, 0, 0, 0);
					Item = BotItem;
					Graphics()->QuadsDraw(&Item, 1);
					Graphics()->SetColor(pInfo->m_BotColor.r, pInfo->m_BotColor.g, pInfo->m_BotColor.b, pInfo->m_BotColor.a);
					SelectSprite(client_data7::SPRITE_TEE_BOT_GLOW, 0, 0, 0);
					Item = BotItem;
					Graphics()->QuadsDraw(&Item, 1);
					Graphics()->QuadsEnd();
				}
				...
				// draw decoration
				if(pInfo->m_aTextures[protocol7::SKINPART_DECORATION] >= 0)
				{
					Graphics()->TextureSet(pInfo->m_aTextures[2]);
					Graphics()->QuadsBegin();
					Graphics()->QuadsSetRotation(pAnim->GetBody()->m_Angle*pi*2);
					Graphics()->SetColor(pInfo->m_aColors[protocol7::SKINPART_DECORATION].r, pInfo->m_aColors[protocol7::SKINPART_DECORATION].g, pInfo->m_aColors[protocol7::SKINPART_DECORATION].b, pInfo->m_aColors[protocol7::SKINPART_DECORATION].a);
					SelectSprite(OutLine ? client_data7::SPRITE_TEE_DECORATION_OUTLINE : client_data7::SPRITE_TEE_DECORATION, 0, 0, 0);
					Item = BodyItem;
					Graphics()->QuadsDraw(&Item, 1);
					Graphics()->QuadsEnd();
				}
				...
				Graphics()->TextureSet(pInfo->m_aTextures[protocol7::SKINPART_BODY]);
				Graphics()->QuadsBegin();
				Graphics()->QuadsSetRotation(pAnim->GetBody()->m_Angle*pi*2);
				if(OutLine)
				{
					Graphics()->SetColor(1.0f, 1.0f, 1.0f, 1.0f);
					SelectSprite(client_data7::SPRITE_TEE_BODY_OUTLINE, 0, 0, 0);
				}
				else
				{
					Graphics()->SetColor(pInfo->m_aColors[protocol7::SKINPART_BODY].r, pInfo->m_aColors[protocol7::SKINPART_BODY].g, pInfo->m_aColors[protocol7::SKINPART_BODY].b, pInfo->m_aColors[protocol7::SKINPART_BODY].a);
					SelectSprite(client_data7::SPRITE_TEE_BODY, 0, 0, 0);
				}
				Item = BodyItem;
				Graphics()->QuadsDraw(&Item, 1);
				Graphics()->QuadsEnd();
				...
				// draw marking
				if(pInfo->m_aTextures[protocol7::SKINPART_MARKING] >= 0 && !OutLine)
				{
					Graphics()->TextureSet(pInfo->m_aTextures[protocol7::SKINPART_MARKING]);
					Graphics()->QuadsBegin();
					Graphics()->QuadsSetRotation(pAnim->GetBody()->m_Angle*pi*2);
					Graphics()->SetColor(pInfo->m_aColors[protocol7::SKINPART_MARKING].r*pInfo->m_aColors[protocol7::SKINPART_MARKING].a, pInfo->m_aColors[protocol7::SKINPART_MARKING].g*pInfo->m_aColors[protocol7::SKINPART_MARKING].a,
						pInfo->m_aColors[protocol7::SKINPART_MARKING].b*pInfo->m_aColors[protocol7::SKINPART_MARKING].a, pInfo->m_aColors[protocol7::SKINPART_MARKING].a);
					SelectSprite(client_data7::SPRITE_TEE_MARKING, 0, 0, 0);
					Item = BodyItem;
					Graphics()->QuadsDraw(&Item, 1);
					Graphics()->QuadsEnd();
				}
				...
				if(!OutLine)
				{
					...
					for(int t = 0; t < 2; t++)
					{
						SelectSprite(t == 0 ? client_data7::SPRITE_TEE_BODY_SHADOW : client_data7::SPRITE_TEE_BODY_UPPER_OUTLINE, 0, 0, 0);
						Item = BodyItem;
						Graphics()->QuadsDraw(&Item, 1);
					}
					Graphics()->QuadsEnd();
				...
				// draw eyes
				Graphics()->TextureSet(pInfo->m_aTextures[protocol7::SKINPART_EYES]);
				Graphics()->QuadsBegin();
				Graphics()->QuadsSetRotation(pAnim->GetBody()->m_Angle*pi*2);
				if(IsBot)
				{
					...
					Graphics()->SetColor(pInfo->m_aColors[protocol7::SKINPART_EYES].r, pInfo->m_aColors[protocol7::SKINPART_EYES].g, pInfo->m_aColors[protocol7::SKINPART_EYES].b, pInfo->m_aColors[protocol7::SKINPART_EYES].a);
				if(p == 1)
				{
					switch (Emote)
					{
						case EMOTE_PAIN:
							SelectSprite(client_data7::SPRITE_TEE_EYES_PAIN, 0, 0, 0);
							break;
						case EMOTE_HAPPY:
							SelectSprite(client_data7::SPRITE_TEE_EYES_HAPPY, 0, 0, 0);
							break;
						case EMOTE_SURPRISE:
							SelectSprite(client_data7::SPRITE_TEE_EYES_SURPRISE, 0, 0, 0);
							break;
						case EMOTE_ANGRY:
							SelectSprite(client_data7::SPRITE_TEE_EYES_ANGRY, 0, 0, 0);
							break;
						default:
							SelectSprite(client_data7::SPRITE_TEE_EYES_NORMAL, 0, 0, 0);
							break;
					}
			...
			// draw feet
			Graphics()->TextureSet(pInfo->m_aTextures[protocol7::SKINPART_FEET]);
			Graphics()->QuadsBegin();
			CAnimKeyframe *pFoot = f ? pAnim->GetFrontFoot() : pAnim->GetBackFoot();
			...
			{
				Graphics()->SetColor(1.0f, 1.0f, 1.0f, 1.0f);
				SelectSprite(client_data7::SPRITE_TEE_FOOT_OUTLINE, 0, 0, 0);
			}
			else
			{
				bool Indicate = !pInfo->m_GotAirJump && g_Config.m_ClAirjumpindicator;
				...
				if(Indicate)
					cs = 0.5f;
				Graphics()->SetColor(pInfo->m_aColors[protocol7::SKINPART_FEET].r*cs, pInfo->m_aColors[protocol7::SKINPART_FEET].g*cs, pInfo->m_aColors[protocol7::SKINPART_FEET].b*cs, pInfo->m_aColors[protocol7::SKINPART_FEET].a);
				SelectSprite(client_data7::SPRITE_TEE_FOOT, 0, 0, 0);
			}
