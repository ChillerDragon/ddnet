		...
		for(int f = 0; f < 2; f++)
		{
			...
			if(f == 1)
			{
				...
				if(IsBot && !OutLine)
				{
					...
					Item = BotItem;
					Graphics()->QuadsDraw(&Item, 1);
					Graphics()->SetColor(pInfo->m_BotColor);
				...
				if(pInfo->m_aTextures[SKINPART_DECORATION].IsValid())
				{
					...
					Graphics()->SetColor(pInfo->m_aColors[SKINPART_DECORATION]);
				...
				{
					Graphics()->SetColor(pInfo->m_aColors[SKINPART_BODY]);
				...
				if(pInfo->m_aTextures[SKINPART_MARKING].IsValid() && !OutLine)
				{
					...
					Graphics()->QuadsBegin();
					Graphics()->QuadsSetRotation(pAnim->GetBody()->m_Angle * pi * 2);
					ColorRGBA MarkingColor = pInfo->m_aColors[SKINPART_MARKING];
				...
				if(IsBot)
				{
					...
					Graphics()->SetColor(pInfo->m_aColors[SKINPART_EYES]);
				if(p == 1)
				{
					switch(Emote)
					{
			...
			else
			{
				...
				if(Indicate)
					ColorScale = 0.5f;
				Graphics()->SetColor(
					pInfo->m_aColors[SKINPART_FEET].r * ColorScale,
					pInfo->m_aColors[SKINPART_FEET].g * ColorScale,
					pInfo->m_aColors[SKINPART_FEET].b * ColorScale,
					pInfo->m_aColors[SKINPART_FEET].a);
