...
{
	if(pInfo->m_aTextures[protocol7::SKINPART_BODY].IsValid())
	...
	for(int p = 0; p < 2; p++)
	{
		...
		for(int f = 0; f < 2; f++)
		{
			...
			if(f == 1)
			{
				...
				// draw decoration
				if(pInfo->m_aTextures[protocol7::SKINPART_DECORATION].IsValid())
				{
				...
				// draw marking
				if(pInfo->m_aTextures[protocol7::SKINPART_MARKING].IsValid() && !OutLine)
				{
			...
			else
			{
				...
				if(Indicate)
					ColorScale = 0.5f;
				Graphics()->SetColor(
					pInfo->m_aColors[protocol7::SKINPART_FEET].r * ColorScale,
					pInfo->m_aColors[protocol7::SKINPART_FEET].g * ColorScale,
					pInfo->m_aColors[protocol7::SKINPART_FEET].b * ColorScale,
					pInfo->m_aColors[protocol7::SKINPART_FEET].a);
