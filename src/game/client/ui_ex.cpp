...
int CUIEx::DoEditBox(void *pID, const CUIRect *pRect, char *pStr, unsigned StrSize, float FontSize, float *Offset, bool Hidden, int Corners, const char *pEmptyText)
{
	...
	if(UI()->LastActiveItem() == pID)
	{
		...
		bool IsShiftPressed = Input()->KeyIsPressed(TW_KEY_LSHIFT) || Input()->KeyIsPressed(TW_KEY_RSHIFT);
		bool IsCtrlPressed = Input()->KeyIsPressed(TW_KEY_LCTRL) || Input()->KeyIsPressed(TW_KEY_RCTRL);

		if(!IsShiftPressed && IsCtrlPressed && Input()->KeyPress(TW_KEY_V))
		{
		...
		if(!IsShiftPressed && IsCtrlPressed && (Input()->KeyPress(TW_KEY_C) || Input()->KeyPress(TW_KEY_X)))
		{
			if(m_HasSelection)
			{
				int SelLeft = minimum(m_CurSelStart, m_CurSelEnd);
		...
		if(!IsShiftPressed && IsCtrlPressed && Input()->KeyPress(TW_KEY_A))
		{
			m_CurSelStart = 0;
		...
		if(!IsShiftPressed && IsCtrlPressed && Input()->KeyPress(TW_KEY_U))
		{
			pStr[0] = '\0';
			m_CurCursor = 0;
			ReturnValue = true;
		...
		for(int i = 0; i < *m_pInputEventCount; i++)
		{
			...
			int NumChars = Len;
			ManipulateChanges = CLineInput::Manipulate(m_pInputEventsArray[i], pStr, StrSize, StrSize, &Len, &m_CurCursor, &NumChars, m_HasSelection ? CLineInput::LINE_INPUT_MODIFY_DONT_DELETE : 0, IsCtrlPressed ? TW_KEY_LCTRL : 0);
