...
int CGameClient::TranslateSnap(CSnapshot *pSnapSix, CSnapshot *pSnapSeven, int Conn, bool Dummy)
{
	...
	for(int i = 0; i < pSnapSix->NumItems(); i++)
	{
		const CSnapshotItem *pItem7 = (CSnapshotItem *)pSnapSix->GetItem(i);
		const int Size = pSnapSix->GetItemSize(i);
		const int ItemType = pSnapSix->GetItemType(i);
	...
	for(int i = 0; i < pSnapSix->NumItems(); i++)
	{
		const CSnapshotItem *pItem7 = pSnapSix->GetItem(i);
		const int Size = pSnapSix->GetItemSize(i);
	...
	return Builder.Finish(pSnapSeven);
}
