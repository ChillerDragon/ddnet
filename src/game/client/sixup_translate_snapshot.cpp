...
int CGameClient::OnDemoRecSnap7(CSnapshot *pFrom, CSnapshot *pTo)
{
	...
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		...
		ClientInfoObj.m_Local = i == Client()->m_TranslationContext.m_aLocalClientId[IClient::CONN_MAIN];
	...
	if(mem_comp(&StandardTuning, &m_aTuning[IClient::CONN_MAIN], sizeof(CTuningParams)) != 0)
	{
		...
		mem_copy(&TuneParams, &m_aTuning[IClient::CONN_MAIN], sizeof(TuneParams));
		mem_copy(pItem, &TuneParams, sizeof(CTuningParams));
	}
