...
void CMenus::RenderDemoPlayer(CUIRect MainView)
{
	...
	if(m_pClient->m_GameConsole.IsClosed() && m_DemoPlayerState == DEMOPLAYER_NONE && g_Config.m_ClDemoKeyboardShortcuts)
	{
		...
		{
			if(Input()->KeyPress(TW_KEY_MOUSE_WHEEL_UP) || Input()->KeyPress(TW_KEY_UP))
			{
				DemoPlayer()->SetSpeedIndex(+1);
				LastSpeedChange = time_get();
			}
			else if(Input()->KeyPress(TW_KEY_MOUSE_WHEEL_DOWN) || Input()->KeyPress(TW_KEY_DOWN))
			{
				DemoPlayer()->SetSpeedIndex(-1);
				LastSpeedChange = time_get();
			}
		...
		// pause/unpause
		if(Input()->KeyPress(TW_KEY_SPACE) || Input()->KeyPress(TW_KEY_RETURN) || Input()->KeyPress(TW_KEY_K))
		{
			if(pInfo->m_Paused)
			{
				DemoPlayer()->Unpause();
		...
		if(Input()->KeyPress(TW_KEY_J))
		{
			DemoPlayer()->SeekTime(-10.0f);
		}
		else if(Input()->KeyPress(TW_KEY_L))
		{
			DemoPlayer()->SeekTime(10.0f);
		}
		else if(Input()->KeyPress(TW_KEY_LEFT))
		{
			DemoPlayer()->SeekTime(-5.0f);
		}
		else if(Input()->KeyPress(TW_KEY_RIGHT))
		{
			DemoPlayer()->SeekTime(5.0f);
		}

		// seek to 0-90%
		const int SeekPercentKeys[] = {TW_KEY_0, TW_KEY_1, TW_KEY_2, TW_KEY_3, TW_KEY_4, TW_KEY_5, TW_KEY_6, TW_KEY_7, TW_KEY_8, TW_KEY_9};
		...
		if(Input()->KeyPress(TW_KEY_HOME))
		{
			DemoPlayer()->SeekPercent(0.0f);
		}
		else if(Input()->KeyPress(TW_KEY_END))
		{
	...
	if(DoButton_DemoPlayer(&s_ExitButton, Localize("Close"), 0, &Button) || (Input()->KeyPress(TW_KEY_C) && m_pClient->m_GameConsole.IsClosed() && m_DemoPlayerState == DEMOPLAYER_NONE))
	{
		Client()->Disconnect();
		DemolistOnUpdate(false);
	}
	...
	int ItemsPerRow, int SelectedIndex, float ScrollValue, bool LogicOnly)
{
	...
	{
		if(Input()->KeyPress(TW_KEY_MOUSE_WHEEL_UP) && UI()->MouseInside(&View))
			gs_ListBoxScrollValue -= Num == 1 ? 0.1f : 3.0f / Num;
		if(Input()->KeyPress(TW_KEY_MOUSE_WHEEL_DOWN) && UI()->MouseInside(&View))
			gs_ListBoxScrollValue += Num == 1 ? 0.1f : 3.0f / Num;
	}
		...
		if(!gs_ListBoxDoneEvents)
		{
			...
			else if(KeyEvents)
			{
				for(int i = 0; i < m_NumInputEvents; i++)
				{
					...
					{
						if(m_aInputEvents[i].m_Key == TW_KEY_DOWN)
							NewIndex = gs_ListBoxNewSelected + 1;
						else if(m_aInputEvents[i].m_Key == TW_KEY_UP)
							NewIndex = gs_ListBoxNewSelected - 1;
						else if(m_aInputEvents[i].m_Key == TW_KEY_PAGEUP)
							NewIndex = maximum(gs_ListBoxNewSelected - 20, 0);
						else if(m_aInputEvents[i].m_Key == TW_KEY_PAGEDOWN)
							NewIndex = minimum(gs_ListBoxNewSelected + 20, gs_ListBoxNumItems - 1);
						else if(m_aInputEvents[i].m_Key == TW_KEY_HOME)
							NewIndex = 0;
						else if(m_aInputEvents[i].m_Key == TW_KEY_END)
...
void CMenus::RenderDemoList(CUIRect MainView)
{
	...
	*/
	...
	if(DoButton_Menu(&s_RefreshButton, Localize("Refresh"), 0, &RefreshRect) || Input()->KeyPress(TW_KEY_F5) || (Input()->KeyPress(TW_KEY_R) && (Input()->KeyIsPressed(TW_KEY_LCTRL) || Input()->KeyIsPressed(TW_KEY_RCTRL))))
	{
		DemolistPopulate();
		DemolistOnUpdate(false);
	}
	...
	if(DoButton_Menu(&s_PlayButton, m_DemolistSelectedIsDir ? Localize("Open") : Localize("Play", "Demo browser"), 0, &PlayRect) || Activated || (Input()->KeyPress(TW_KEY_P) && m_pClient->m_GameConsole.IsClosed() && m_DemoPlayerState == DEMOPLAYER_NONE))
	{
		if(m_DemolistSelectedIndex >= 0)
		{
	...
	if(!m_DemolistSelectedIsDir)
	{
		...
		if(DoButton_Menu(&s_DeleteButton, Localize("Delete"), 0, &DeleteRect) || m_DeletePressed || (Input()->KeyPress(TW_KEY_D) && m_pClient->m_GameConsole.IsClosed()))
		{
			if(m_DemolistSelectedIndex >= 0)
			{
				UI()->SetActiveItem(0);
		...
		if(DoButton_Menu(&s_RenderButton, Localize("Render"), 0, &RenderRect) || (Input()->KeyPress(TW_KEY_R) && m_pClient->m_GameConsole.IsClosed()))
		{
			if(m_DemolistSelectedIndex >= 0)
			{
				UI()->SetActiveItem(0);
