...
void CMenus::RenderSettingsTee7(CUIRect MainView)
{
	...
	// Preview
	{
...

		{
			...
			RenderTools()->RenderTee7(CAnimState::GetIdle(), &OwnSkinInfo, TeeEmote, TeeDirection, TeePosition);
		...
		RenderTools()->RenderTee7(CAnimState::GetIdle(), &TeamSkinInfo, 0, vec2(1, 0), vec2(TeeLeft.x + TeeLeft.w / 2.0f, TeeLeft.y + TeeLeft.h / 2.0f + 6.0f));
		...
		RenderTools()->RenderTee7(CAnimState::GetIdle(), &TeamSkinInfo, 0, vec2(-1, 0), vec2(TeeRight.x + TeeRight.w / 2.0f, TeeRight.y + TeeRight.h / 2.0f + 6.0f));
	}
	...
	for(int i = 0; i < (int)s_paSkinList.size(); ++i)
	{
		...
		if(Item.m_Visible)
		{
...

			{
				...
				RenderTools()->RenderTee7(CAnimState::GetIdle(), &Info, TeeEmote, vec2(1.0f, 0.0f), vec2(Item.m_Rect.x + Item.m_Rect.w / 2, Item.m_Rect.y + Item.m_Rect.h / 2));
			}
...
void CMenus::RenderSkinPartSelection(CUIRect MainView)
{
	...
	for(int i = 0; i < (int)s_paList[m_TeePartSelected].size(); ++i)
	{
		...
		if(Item.m_Visible)
		{
			...
			RenderTools()->RenderTee7(CAnimState::GetIdle(), &Info, TeePartEmote, vec2(1.0f, 0.0f), TeePos);
