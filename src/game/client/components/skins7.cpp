...
const char * const CSkins7::ms_apSkinPartNames[protocol7::NUM_SKINPARTS] = {"body", "marking", "decoration", "hands", "feet", "eyes"}; /* Localize("body","skins7");Localize("marking","skins7");Localize("decoration","skins7");Localize("hands","skins7");Localize("feet","skins7");Localize("eyes","skins7"); */
const char * const CSkins7::ms_apColorComponents[NUM_COLOR_COMPONENTS] = {"hue", "sat", "lgt", "alp"};

char *CSkins7::ms_apSkinVariables[protocol7::NUM_SKINPARTS] = {0};
int *CSkins7::ms_apUCCVariables[protocol7::NUM_SKINPARTS] = {0};
int *CSkins7::ms_apColorVariables[protocol7::NUM_SKINPARTS] = {0};
...
int CSkins7::SkinPartScan(const char *pName, int IsDir, int DirType, void *pUser)
{
	...
	if(!pSelf->Graphics()->LoadPNG(&Info, aBuf, DirType))
	{
		...
		pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_ADDINFO, "skins7", aBuf);
		return 0;
	}
...
void CSkins7::OnInit()
{
	ms_apSkinVariables[protocol7::SKINPART_BODY] = g_Config.m_PlayerSkinBody;
	ms_apSkinVariables[protocol7::SKINPART_MARKING] = g_Config.m_PlayerSkinMarking;
	ms_apSkinVariables[protocol7::SKINPART_DECORATION] = g_Config.m_PlayerSkinDecoration;
	ms_apSkinVariables[protocol7::SKINPART_HANDS] = g_Config.m_PlayerSkinHands;
	ms_apSkinVariables[protocol7::SKINPART_FEET] = g_Config.m_PlayerSkinFeet;
	ms_apSkinVariables[protocol7::SKINPART_EYES] = g_Config.m_PlayerSkinEyes;
	ms_apUCCVariables[protocol7::SKINPART_BODY] = &g_Config.m_PlayerUseCustomColorBody;
	ms_apUCCVariables[protocol7::SKINPART_MARKING] = &g_Config.m_PlayerUseCustomColorMarking;
	ms_apUCCVariables[protocol7::SKINPART_DECORATION] = &g_Config.m_PlayerUseCustomColorDecoration;
	ms_apUCCVariables[protocol7::SKINPART_HANDS] = &g_Config.m_PlayerUseCustomColorHands;
	ms_apUCCVariables[protocol7::SKINPART_FEET] = &g_Config.m_PlayerUseCustomColorFeet;
	ms_apUCCVariables[protocol7::SKINPART_EYES] = &g_Config.m_PlayerUseCustomColorEyes;
	ms_apColorVariables[protocol7::SKINPART_BODY] = &g_Config.m_PlayerColorBody;
	ms_apColorVariables[protocol7::SKINPART_MARKING] = &g_Config.m_PlayerColorMarking;
	ms_apColorVariables[protocol7::SKINPART_DECORATION] = &g_Config.m_PlayerColorDecoration;
	ms_apColorVariables[protocol7::SKINPART_HANDS] = &g_Config.m_PlayerColorHands;
	ms_apColorVariables[protocol7::SKINPART_FEET] = &g_Config.m_PlayerColorFeet;
	ms_apColorVariables[protocol7::SKINPART_EYES] = &g_Config.m_PlayerColorEyes;
	...
	for(int p = 0; p < protocol7::NUM_SKINPARTS; p++)
	{
		...
		m_pClient->m_Menus.RenderLoading();
	}
...
void CSkins7::RandomizeSkin()
{
	for(int p = 0; p < protocol7::NUM_SKINPARTS; p++)
	{
		...
		int Alp = 0;
		if (p == 1) // protocol7::SKINPART_MARKING
			Alp = random_int() % 255;
	...
	for(int p = 0; p < protocol7::NUM_SKINPARTS; p++)
	{
		const CSkins7::CSkinPart *s = GetSkinPart(p, random_int() % NumSkinPart(p));
...
bool CSkins7::ValidateSkinParts(char* aPartNames[protocol7::NUM_SKINPARTS], int* aUseCustomColors, int* aPartColors, int GameFlags) const
{
	// force standard (black) eyes on team skins
	if(GameFlags&GAMEFLAG_TEAMS)
	{
		// TODO: adjust eye color here as well?
		if(str_comp(aPartNames[protocol7::SKINPART_EYES], "colorable") == 0 || str_comp(aPartNames[protocol7::SKINPART_EYES], "negative") == 0)
		{
			str_copy(aPartNames[protocol7::SKINPART_EYES], "standard", MAX_SKIN_ARRAY_SIZE);
			return false;
		}
	}
	else
	{
		const int BodyColor = aPartColors[protocol7::SKINPART_BODY];
		const int EyeColor = aPartColors[protocol7::SKINPART_EYES];
		...
		if(!aUseCustomColors[protocol7::SKINPART_BODY])
			BodyHsl = vec3(0, 0, 1);

		vec3 BodyRgb = HslToRgb(BodyHsl);
		vec3 BodyLab = RgbToLab(BodyRgb);

		if(str_comp(aPartNames[protocol7::SKINPART_EYES], "negative") == 0)
		{
			if(!aUseCustomColors[protocol7::SKINPART_EYES])
				EyeHsl = vec3(0, 0, 1);
			...
			if(Dist < MIN_EYE_BODY_COLOR_DIST)
			{
				...
				if(OrgEyeHsl.l < DARKEST_COLOR_LGT/255.f)
					str_copy(aPartNames[protocol7::SKINPART_EYES], "standard", MAX_SKIN_ARRAY_SIZE); // black
				else
				{
					aUseCustomColors[protocol7::SKINPART_EYES] = 1;
					aPartColors[protocol7::SKINPART_EYES] = (int(OrgEyeHsl.h*255) << 16) | (int(OrgEyeHsl.s*255) << 8) | (int(OrgEyeHsl.l*255));
				}

				return false;
			}
		}
		else if(str_comp(aPartNames[protocol7::SKINPART_EYES], "colorable") == 0)
		{
			if(!aUseCustomColors[protocol7::SKINPART_EYES])
				EyeHsl = vec3(0, 0, 1);
...
void CSkins7::SaveSkinfile(const char *pSaveSkinName)
{
	// char aBuf[256];
