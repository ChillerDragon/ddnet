...
int CSkins7::SkinPartScan(const char *pName, int IsDir, int DirType, void *pUser)
{
	...
	Part.m_OrgTexture = pSelf->Graphics()->LoadTextureRaw(Info, 0);
	...
	Part.m_ColorTexture = pSelf->Graphics()->LoadTextureRaw(Info, 0);
	free(Info.m_pData);
...
void CSkins7::OnInit()
{
	...
	for(int p = 0; p < protocol7::NUM_SKINPARTS; p++)
	{
		...
		if(p == protocol7::SKINPART_MARKING || p == protocol7::SKINPART_DECORATION)
		{
			...
			m_avSkinParts[p].emplace_back(NoneSkinPart);
		}
		...
		if(m_avSkinParts[p].empty())
		{
			...
			m_avSkinParts[p].emplace_back(DummySkinPart);
		}
...

	{
		...
		else
		{
			...
			Console()->Print(IConsole::OUTPUT_LEVEL_ADDINFO, "skins7", aBuf);
			m_XmasHatTexture = Graphics()->LoadTextureRawMove(Info, 0);
		}
	}
	GameClient()->m_Menus.RenderLoading(Localize("Loading DDNet Client"), Localize("Loading skin files"), 0);

	{
		...
		else
		{
			...
			Console()->Print(IConsole::OUTPUT_LEVEL_ADDINFO, "skins7", aBuf);
			m_BotTexture = Graphics()->LoadTextureRawMove(Info, 0);
		}
	}
	GameClient()->m_Menus.RenderLoading(Localize("Loading DDNet Client"), Localize("Loading skin files"), 0);
}
