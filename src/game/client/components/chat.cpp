...
bool CChat::OnInput(IInput::CEvent Event)
{
	...
	if(Input()->KeyIsPressed(TW_KEY_LCTRL) && Input()->KeyPress(TW_KEY_V))
	{
	...
	if(Input()->KeyIsPressed(TW_KEY_LCTRL)) // jump to spaces and special ASCII characters
	{
		int SearchDirection = 0;
		if(Input()->KeyPress(TW_KEY_LEFT) || Input()->KeyPress(TW_KEY_BACKSPACE))
			SearchDirection = -1;
		else if(Input()->KeyPress(TW_KEY_RIGHT) || Input()->KeyPress(TW_KEY_DELETE))
			SearchDirection = 1;
		...
		if(SearchDirection != 0)
		{
			...
			if(Input()->KeyPress(TW_KEY_BACKSPACE))
			{
			...
			else if(Input()->KeyPress(TW_KEY_DELETE))
			{
	...
	if(Event.m_Flags & IInput::FLAG_PRESS && Event.m_Key == TW_KEY_ESCAPE)
	{
		DisableMode();
		m_pClient->OnRelease();
	...
	else if(Event.m_Flags & IInput::FLAG_PRESS && (Event.m_Key == TW_KEY_RETURN || Event.m_Key == TW_KEY_KP_ENTER))
	{
	...
	if(Event.m_Flags & IInput::FLAG_PRESS && Event.m_Key == TW_KEY_TAB)
	{
	...
	if(Event.m_Flags & IInput::FLAG_PRESS && Event.m_Key == TW_KEY_LSHIFT)
	{
		m_ReverseTAB = true;
	}
	else if(Event.m_Flags & IInput::FLAG_RELEASE && Event.m_Key == TW_KEY_LSHIFT)
	{
		m_ReverseTAB = false;
	}
	if(Event.m_Flags & IInput::FLAG_PRESS && Event.m_Key == TW_KEY_UP)
	{
		if(m_pHistoryEntry)
		{
	...
	else if(Event.m_Flags & IInput::FLAG_PRESS && Event.m_Key == TW_KEY_DOWN)
	{
		if(m_pHistoryEntry)
