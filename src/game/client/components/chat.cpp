		...
		if(pCurrentLine->m_ClientId >= 0 && pCurrentLine->m_aName[0] != '\0')
		{
			if(!g_Config.m_ClChatOld)
			{
				...
				if(Client()->IsSixup())
				{
					...
					{
						const char *pPartName = m_pClient->m_aClients[pCurrentLine->m_ClientId].m_Sixup.m_aaSkinPartNames[Part];
						pCurrentLine->m_Sixup.m_aUseCustomColors[Part] = m_pClient->m_aClients[pCurrentLine->m_ClientId].m_Sixup.m_aUseCustomColors[Part];
						pCurrentLine->m_Sixup.m_aSkinPartColors[Part] = m_pClient->m_aClients[pCurrentLine->m_ClientId].m_Sixup.m_aSkinPartColors[Part];
						str_utf8_copy_num(
						...
						if(m_pClient->m_aClients[pCurrentLine->m_ClientId].m_SkinInfo.m_Sixup.m_HatTexture.IsValid())
						{
							...
							if(Part == protocol7::SKINPART_DECORATION && str_comp(pPartName, "twinbopp"))
								pCurrentLine->m_Sixup.m_HatSpriteIndex = CSkins7::HAT_OFFSET_SIDE + (ClientId % CSkins7::HAT_NUM);
							pCurrentLine->m_Sixup.m_HatTexture = m_pClient->m_aClients[pCurrentLine->m_ClientId].m_SkinInfo.m_Sixup.m_HatTexture;
						}
					}
				}
			}
