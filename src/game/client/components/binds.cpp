...
{
	int Mask = 0;
	Mask |= i->KeyIsPressed(TW_KEY_LSHIFT) << CBinds::MODIFIER_SHIFT;
	Mask |= i->KeyIsPressed(TW_KEY_RSHIFT) << CBinds::MODIFIER_SHIFT;
	Mask |= i->KeyIsPressed(TW_KEY_LCTRL) << CBinds::MODIFIER_CTRL;
	Mask |= i->KeyIsPressed(TW_KEY_RCTRL) << CBinds::MODIFIER_CTRL;
	Mask |= i->KeyIsPressed(TW_KEY_LALT) << CBinds::MODIFIER_ALT;
	if(!Mask)
...
bool CBinds::ModifierMatchesKey(int Modifier, int Key)
{
	...
	{
	case MODIFIER_SHIFT:
		return Key == TW_KEY_LSHIFT || Key == TW_KEY_RSHIFT;
	case MODIFIER_CTRL:
		return Key == TW_KEY_LCTRL || Key == TW_KEY_RCTRL;
	case MODIFIER_ALT:
		return Key == TW_KEY_LALT;
	case MODIFIER_NONE:
	default:
		return false;
	}
...
bool CBinds::OnInput(IInput::CEvent e)
{
	...
	if(e.m_Key <= 0 || e.m_Key >= TW_KEY_LAST)
		return false;
...
{
	if(KeyID > 0 && KeyID < TW_KEY_LAST && m_aapKeyBindings[Modifier][KeyID])
...
void CBinds::GetKey(const char *pBindStr, char *aBuf, unsigned BufSize)
{
	...
	{
		for(int KeyId = 0; KeyId < TW_KEY_LAST; KeyId++)
		{
...
void CBinds::SetDefaults()
{
	...
	UnbindAll();
	Bind(TW_KEY_F1, "toggle_local_console");
	Bind(TW_KEY_F2, "toggle_remote_console");
	Bind(TW_KEY_TAB, "+scoreboard");
	Bind(TW_KEY_EQUALS, "+statboard");
	Bind(TW_KEY_F10, "screenshot");

	Bind(TW_KEY_A, "+left");
	Bind(TW_KEY_D, "+right");

	Bind(TW_KEY_SPACE, "+jump");
	Bind(TW_KEY_MOUSE_1, "+fire");
	Bind(TW_KEY_MOUSE_2, "+hook");
	Bind(TW_KEY_LSHIFT, "+emote");
	Bind(TW_KEY_RETURN, "+show_chat; chat all");
	Bind(TW_KEY_RIGHT, "spectate_next");
	Bind(TW_KEY_LEFT, "spectate_previous");
	Bind(TW_KEY_RSHIFT, "+spectate");

	Bind(TW_KEY_1, "+weapon1");
	Bind(TW_KEY_2, "+weapon2");
	Bind(TW_KEY_3, "+weapon3");
	Bind(TW_KEY_4, "+weapon4");
	Bind(TW_KEY_5, "+weapon5");

	Bind(TW_KEY_MOUSE_WHEEL_UP, "+prevweapon");
	Bind(TW_KEY_MOUSE_WHEEL_DOWN, "+nextweapon");

	Bind(TW_KEY_T, "+show_chat; chat all");
	Bind(TW_KEY_Y, "+show_chat; chat team");
	Bind(TW_KEY_U, "+show_chat");
	Bind(TW_KEY_I, "+show_chat; chat all /c ");

	Bind(TW_KEY_F3, "vote yes");
	Bind(TW_KEY_F4, "vote no");

	Bind(TW_KEY_K, "kill");
	Bind(TW_KEY_Q, "say /pause");
	Bind(TW_KEY_P, "say /pause");
...
void CBinds::SetDDRaceBinds(bool FreeOnly)
{
	...
	{
		Bind(TW_KEY_KP_PLUS, "zoom+", FreeOnly);
		Bind(TW_KEY_KP_MINUS, "zoom-", FreeOnly);
		Bind(TW_KEY_KP_MULTIPLY, "zoom", FreeOnly);
		Bind(TW_KEY_PAUSE, "say /pause", FreeOnly);
		Bind(TW_KEY_UP, "+jump", FreeOnly);
		Bind(TW_KEY_LEFT, "+left", FreeOnly);
		Bind(TW_KEY_RIGHT, "+right", FreeOnly);
		Bind(TW_KEY_LEFTBRACKET, "+prevweapon", FreeOnly);
		Bind(TW_KEY_RIGHTBRACKET, "+nextweapon", FreeOnly);
		Bind(TW_KEY_C, "say /rank", FreeOnly);
		Bind(TW_KEY_V, "say /info", FreeOnly);
		Bind(TW_KEY_B, "say /top5", FreeOnly);
		Bind(TW_KEY_S, "+showhookcoll", FreeOnly);
		Bind(TW_KEY_X, "toggle cl_dummy 0 1", FreeOnly);
		Bind(TW_KEY_H, "toggle cl_dummy_hammer 0 1", FreeOnly);
		Bind(TW_KEY_SLASH, "+show_chat; chat all /", FreeOnly);
		Bind(TW_KEY_PAGEDOWN, "toggle cl_show_quads 0 1", FreeOnly);
		Bind(TW_KEY_PAGEUP, "toggle cl_overlay_entities 0 100", FreeOnly);
		Bind(TW_KEY_KP_0, "say /emote normal 999999", FreeOnly);
		Bind(TW_KEY_KP_1, "say /emote happy 999999", FreeOnly);
		Bind(TW_KEY_KP_2, "say /emote angry 999999", FreeOnly);
		Bind(TW_KEY_KP_3, "say /emote pain 999999", FreeOnly);
		Bind(TW_KEY_KP_4, "say /emote surprise 999999", FreeOnly);
		Bind(TW_KEY_KP_5, "say /emote blink 999999", FreeOnly);
		Bind(TW_KEY_MOUSE_3, "+spectate", FreeOnly);
		Bind(TW_KEY_MINUS, "spectate_previous", FreeOnly);
		Bind(TW_KEY_EQUALS, "spectate_next", FreeOnly);
	}
