...
void CScoreboard::RenderScoreboard(float x, float y, float w, int Team, const char *pTitle, int NumPlayers)
{
	...
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		...
		if(RenderDead)
		{
			...
			if(m_pClient->m_Snap.m_pGameInfoObj->m_GameFlags & GAMEFLAG_TEAMS)
			{
				vec4 Color = m_pClient->m_Skins7.GetColor(m_pClient->m_Skins7.GetTeamColor(true, 0, m_pClient->m_aClients[pInfo->m_ClientId].m_Team, protocol7::SKINPART_BODY), false);
