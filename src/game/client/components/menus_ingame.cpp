...
void CMenus::RenderServerControl(CUIRect MainView)
{
	...
	// vote menu
	{
		...
		// render quick search
		{
			...
			if(m_ControlPageOpening || (Input()->KeyPress(TW_KEY_F) && (Input()->KeyIsPressed(TW_KEY_LCTRL) || Input()->KeyIsPressed(TW_KEY_RCTRL))))
			{
		...
		if(Input()->KeyPress(TW_KEY_R) && (Input()->KeyIsPressed(TW_KEY_LCTRL) || Input()->KeyIsPressed(TW_KEY_RCTRL)))
...
void CMenus::RenderGhost(CUIRect MainView)
{
	...
	if(DoButton_Menu(&s_ReloadButton, Localize("Reload"), 0, &Button) || Input()->KeyPress(TW_KEY_F5))
	{
