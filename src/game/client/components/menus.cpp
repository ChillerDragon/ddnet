	...
	*NewModifier = Modifier;
	...
	if(UI()->ActiveItem() == pID)
	{
		if(m_Binder.m_GotKey)
		{
			...
			if(m_Binder.m_Key.m_Key != TW_KEY_ESCAPE)
			{
...
bool CMenus::OnInput(IInput::CEvent e)
{
	...
	if(IsActive())
	{
		...
		{
			// special for popups
			if(e.m_Key == TW_KEY_RETURN || e.m_Key == TW_KEY_KP_ENTER)
				m_EnterPressed = true;
			else if(e.m_Key == TW_KEY_DELETE)
...
{
	return m_Popup == POPUP_NONE &&
	       !Input()->KeyIsPressed(TW_KEY_LSHIFT) && !Input()->KeyIsPressed(TW_KEY_RSHIFT) && !Input()->KeyIsPressed(TW_KEY_LCTRL) && !Input()->KeyIsPressed(TW_KEY_RCTRL) && !Input()->KeyIsPressed(TW_KEY_LALT) && // no modifier
	...
	for(int i = 0; i < m_NumInputEvents; i++)
	{
		if(m_aInputEvents[i].m_Flags & IInput::FLAG_PRESS)
		{
			if(UI()->LastActiveItem() == &g_Config.m_UiServerAddress)
				return false;
			else if(m_aInputEvents[i].m_Key == TW_KEY_DOWN)
