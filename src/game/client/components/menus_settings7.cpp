...
void CMenus::RenderSettingsTee7(CUIRect MainView)
{
	...
	// Preview
	{
		...
		for(int p = 0; p < protocol7::NUM_SKINPARTS; p++)
		{
			...
			{
				TeamSkinInfo.m_Sixup.m_aTextures[p] = pSkinPart->m_OrgTexture;
				TeamSkinInfo.m_Sixup.m_aColors[p] = vec4(1.0f, 1.0f, 1.0f, 1.0f);
			}
		}
		...
		for(int p = 0; p < protocol7::NUM_SKINPARTS; p++)
		{
			...
			TeamSkinInfo.m_Sixup.m_aColors[p] = m_pClient->m_Skins7.GetColorV4(TeamColor, p == protocol7::SKINPART_MARKING);
		}
	...
	for(int i = 0; i < (int)s_vpSkinList.size(); ++i)
	{
		...
		if(Item.m_Visible)
		{
			...
			for(int p = 0; p < protocol7::NUM_SKINPARTS; p++)
			{
				...
				{
					Info.m_Sixup.m_aTextures[p] = s->m_apParts[p]->m_OrgTexture;
					Info.m_Sixup.m_aColors[p] = vec4(1.0f, 1.0f, 1.0f, 1.0f);
				}
			}
