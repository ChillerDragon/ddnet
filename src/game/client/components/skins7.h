...
public:
	...
	static char *ms_apSkinVariables[protocol7::NUM_SKINPARTS];
	static int *ms_apUCCVariables[protocol7::NUM_SKINPARTS]; // use custom color
	static int *ms_apColorVariables[protocol7::NUM_SKINPARTS];
	...
	bool ValidateSkinParts(char *aPartNames[protocol7::NUM_SKINPARTS], int *aUseCustomColors, int* aPartColors, int GameFlags) const;

	void SaveSkinfile(const char *pSaveSkinName);

private:
	int m_ScanningPart;
	sorted_array<CSkinPart> m_aaSkinParts[protocol7::NUM_SKINPARTS];
