...
void CGameContext::ConPracticeCmdList(IConsole::IResult *pResult, void *pUserData)
{
	...
	for(const IConsole::CCommandInfo *pCmd = pSelf->Console()->FirstCommandInfo(pResult->m_ClientId, IConsole::ACCESS_LEVEL_USER, CMDFLAG_PRACTICE);
		pCmd; pCmd = pCmd->NextCommandInfo(pResult->m_ClientId, IConsole::ACCESS_LEVEL_USER, CMDFLAG_PRACTICE))
	{
		char aCommand[64];

		str_format(aCommand, sizeof(aCommand), "/%s%s", pCmd->m_pName, pCmd->NextCommandInfo(pResult->m_ClientId, IConsole::ACCESS_LEVEL_USER, CMDFLAG_PRACTICE) ? ", " : "");
