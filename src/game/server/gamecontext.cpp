...
void CGameContext::AddVote(const char *pDescription, const char *pCommand)
{
	...
	{
		log_info("server", "maximum number of vote options reached");
		return;
	}
	...
	{
		log_info("server", "skipped invalid command '%s'", pCommand);
		return;
	}
	while(*pDescription == ' ')
		pDescription++;
	...
	{
		log_info("server", "skipped invalid option '%s'", pDescription);
		return;
	}
	...
	while(pOption)
	{
		...
		{
			log_info("server", "option '%s' already exists", pDescription);
			return;
		}
		pOption = pOption->m_pNext;
	}
...
void CGameContext::ConRemoveVote(IConsole::IResult *pResult, void *pUserData)
{
	...
	if(!pOption)
	{
		log_info("server", "option '%s' does not exist", pDescription);
		return;
	}
...
void CGameContext::ConForceVote(IConsole::IResult *pResult, void *pUserData)
{
	...
	if(str_comp_nocase(pType, "option") == 0)
	{
		...
		if(!pOption)
		{
			log_info("server", "'%s' isn't an option on this server", pValue);
			return;
		}
	}
	else if(str_comp_nocase(pType, "kick") == 0)
	{
		...
		{
			log_info("server", "Your client does not see the real client IDs of this server. Use a more recent DDNet client.");
			return;
		}
		int KickId = str_toint(pValue);
		if(KickId < 0 || KickId >= MAX_CLIENTS || !pSelf->m_apPlayers[KickId])
		{
			log_info("server", "Invalid client id to kick");
			return;
		}
