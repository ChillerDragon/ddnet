...
void CGameControllerDDRace::HandleCharacterTiles(CCharacter *pChr, int MapIndex)
{
	...
	// unlock team
	else if(((m_TileIndex == TILE_UNLOCK_TEAM) || (m_TileFIndex == TILE_UNLOCK_TEAM)) && Teams().TeamLocked(GetPlayerTeam(ClientID)))
	{
		Teams().SetTeamLock(GetPlayerTeam(ClientID), false);
...
CClientMask CGameControllerDDRace::GetMaskForPlayerWorldEvent(int Asker, int ExceptID)
{
	...
	return Teams().TeamMask(GetPlayerTeam(Asker), ExceptID, Asker);
}
