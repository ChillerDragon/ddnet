...
- Avoid object-like `#define` directives for constants. Use `enum class` or `inline constexpr` instead.
  Only use object-like `#define` directives if there is no other option (e.g. for conditional compilation).
