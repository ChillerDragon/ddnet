...
then
	mkdir "$build_dir"
	cd "$build_dir"
	cmake -DCMAKE_BUILD_TYPE=Debug -DDOWNLOAD_GTEST=ON ..
	make -j"$ARG_JOBS"
	cd ..
fi
# generated code hack
...
check_file() {
	local source_file="$1"
	local build_dir="$2"
	clang-tidy --config-file=scripts/clang-tidy-camel-case.yml "$source_file" -p "$build_dir" 2>/dev/null
}
...
multi_thread_bash() {
	...
	list_code_files | while read -r source_file
		do
			...
			do
				log "$(jobs | wc -l) jobs running waiting ..."
				sleep 0.5
			done
			check_file "$source_file" "$build_dir" &
		done
