...
def generated_code_hack(build_dir: str) -> None:
    ...
    for gen_file in ('client_data.h', 'client_data7.h'):
        shutil.copy(os.path.join(build_dir, 'src/game/generated',
                    gen_file), 'src/game/generated')


def main() -> int:
    p = argparse.ArgumentParser(description="")
    p.add_argument('--skip', help="", default=0, type=int)
    p.add_argument('-j', '--jobs', help="",
                   default=multiprocessing.cpu_count(), type=int)
    p.add_argument('-n', '--dry-run', action='store_true',
                   help="Don't fix, only warn", default=False)
    p.add_argument('build_dir', help="",
                   default='build-camel', nargs='?', type=str)
    ...
    with multiprocessing.Pool(args.jobs) as pool:
        check_file_prefilled = functools.partial(
            check_file,
            skip=args.skip,
            total=total,
        ...
        results = pool.starmap(check_file_prefilled,
